...
HISTORY -- 2026-08-10 run: test_admin_question_detail_view_modal_opens_and_closes
and the AI-generate lifecycle test both skipped on the first run. Diagnostics
showed /admin/questions rendered fine with a real 'Add Question' control visible,
but _question_rows() -- which searched plain 'tr' -- was almost certainly matching
a header <tr> (no <td> action buttons) as "row 0" instead of a real data row.
_question_rows() now prefers 'tbody tr' and filters any 'tr' fallback down to rows
that actually contain <td> cells, so it can no longer land on a header row.
"""
import time
import uuid
...
def _question_rows(driver):
    """Data rows in the questions table -- prefers tbody rows, and any plain
    'tr' fallback is filtered to rows that actually contain <td> cells so a
    header row (no action buttons) can never be picked up as "row 0"."""
    rows = [r for r in driver.find_elements(By.CSS_SELECTOR, 'tbody tr') if r.is_displayed()]
    if not rows:
        rows = [
            r for r in driver.find_elements(By.CSS_SELECTOR, 'tr')
            if r.is_displayed() and r.find_elements(By.CSS_SELECTOR, 'td')
        ]
    if rows:
        return rows
...
def test_admin_ai_generated_question_edit_preview_delete_lifecycle(
    ...
    time.sleep(3)
    # If the click didn't register (e.g. an overlay intercepted it), retry once via JS.
    still_on_step = _find_by_text(driver, 'button', ['generate with ai'])
    if still_on_step:
        try:
            driver.execute_script('arguments[0].click();', still_on_step[0])
        except Exception:
            pass
